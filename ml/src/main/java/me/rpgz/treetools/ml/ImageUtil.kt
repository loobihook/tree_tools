...
fun preProcess(bitmapIn: Bitmap): FloatBuffer {
    val bitmap = bitmapIn.takeIf { it.config != Bitmap.Config.HARDWARE }
        ?: bitmapIn.copy(Bitmap.Config.ARGB_8888, true)

    val floatCount = DIM_BATCH_SIZE * DIM_PIXEL_SIZE * IMAGE_SIZE_X * IMAGE_SIZE_Y
    val bb = ByteBuffer
        .allocateDirect(floatCount * java.lang.Float.BYTES)
        .order(ByteOrder.nativeOrder())
    val imgData = bb.asFloatBuffer()
    imgData.rewind()
